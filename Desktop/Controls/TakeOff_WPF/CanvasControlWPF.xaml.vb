'Filename: Desktop/Controls/TakeOff_WPF/CanvasControlWPF.vb
#Region "Info. & Imports"
'Option Strict On
...
''' </summary>
''' <remarks>
''' CanvasControlWPF is the primary UI component for drawing operations.
''' It manages:
...
#End Region
    ...
    ''' </remarks>
    Private _currentLayoutWPF As CanvasLayout
    ...
    Private _showRulersWPF As Boolean = True

    Private _backBufferWPF As Bitmap = Nothing
    Private _backGraphicsWPF As Graphics = Nothing
...
#Region "Private Variables"
    ...
    Private myCoordinatesBox As CoordinatesBoxWPF = New CoordinatesBoxWPF(Me)
...
#Region "Video double buffering management"
    ...
    '''' </summary>
    Private myRefreshBackBufferWPF As Bitmap

    ''' <summary>Bitmap that forms the second-level video buffer (persistent until Redraw())
    ''' </summary>
    Private myRedrawBackBufferWPF As Bitmap
#End Region

#Region "Various"
    Private myIsDraggingWPF As Boolean = False
    Private myIsLoadedWPF As Boolean = False
    Private myRulersWPF As New RulersWPF(Me)
    Private myLastMouseDownPointWPF As Point
    Public ReadOnly Property LastMouseDownLogicalCoord() As Point
        Get
            Return myLastMouseDownPointWPF
        End Get
    End Property
    Private WithEvents myDistanceRulerWPF As DistanceRuler = New DistanceRuler(Me)
    Private myIsLayoutSuspendedWPF As Boolean = True
#End Region
...
#Region "Flag for display"
    Private myShowMouseCoordinatesWPF As Boolean = True
    Private myShowRulersWPF As Boolean = True
    Private myIsChangingAutoScrollWPF As Boolean = False
    ...
    <Description("Allows you to view the coordinates at which the mouse is located"),
     DefaultValue(True)>
    Public Property ShowMouseCoordinatesWPF() As Boolean
        Get
            Return myShowMouseCoordinatesWPF
        End Get
        Set(ByVal value As Boolean)
            myShowMouseCoordinatesWPF = value
        End Set
    End Property
    ...
    Public Property ShowRulers() As Boolean
        Get
            Return myShowRulersWPF
        End Get
        Set(ByVal Value As Boolean)
            myShowRulersWPF = Value
        End Set
    End Property
#End Region

#Region "Selection/zoom box"
    Private mySelectionBox As New SelectionBoxElementWPF()
...
#Region "Routine to calculate/display logical windows"
    ...
    Private Function VisibleAreaToLogicalArea(ByVal visibleArea As RECT, Optional ByVal CenterWindow As Boolean = True,
        ...
        If myShowRulersWPF AndAlso ExcludeRulersArea Then
            rulersPhysicalSize = myRulersWPF.Size
        End If
...
#Region "Origin, coordinates, dimensions, etc."
    ...
    Public Property ScaleFactor() As Single
        Get
            Return GraphicInfoWPF.ScaleFactor
        End Get
        Set(ByVal Value As Single)
            ' Se il nuovo valore mi porterebbe fuori dai limiti minimi o massimi, lo ignoro
            If (Width / Value) > MaxLogicalWindowSize.Width AndAlso Value < GraphicInfoWPF.ScaleFactor Then Exit Property
            If (Height / Value) > MaxLogicalWindowSize.Height AndAlso Value < GraphicInfoWPF.ScaleFactor Then Exit Property
            If (Width / Value) < MinLogicalWindowSize.Width AndAlso Value > GraphicInfoWPF.ScaleFactor Then Exit Property
            If (Height / Value) < MinLogicalWindowSize.Height AndAlso Value > GraphicInfoWPF.ScaleFactor Then Exit Property
            ' Aggiorno i dati interni
            GraphicInfoWPF.ScaleFactor = Value
        End Set
    End Property
    ...
    Public Property LogicalOrigin() As System.Drawing.Point
        Get
            Return GraphicInfoWPF.LogicalOrigin
        End Get
        Set(ByVal Value As System.Drawing.Point)
            GraphicInfoWPF.LogicalOrigin = Value
        End Set
    End Property
    ...
    Public Property LogicalWidth() As Integer
        Get
            Return GraphicInfoWPF.LogicalWidth
        End Get
        Set(ByVal Value As Integer)
            GraphicInfoWPF.LogicalWidth = Value
        End Set
    End Property
    ...
    Public Property LogicalHeight() As Integer
        Get
            Return GraphicInfoWPF.LogicalHeight
        End Get
        Set(ByVal Value As Integer)
            GraphicInfoWPF.LogicalHeight = Value
        End Set
    End Property
    ...
    <Browsable(False)>
    Public Property LogicalAreaWPF() As RECT
        Get
            Return GraphicInfoWPF.LogicalArea
        End Get
        Private Set(ByVal value As RECT)
            GraphicInfoWPF.LogicalArea = value
        End Set
    End Property
    ...
    Public Property GraphicInfoWPF() As ConversionInfo
        Get
            Return myGraphicInfoWPF
        End Get
        Private Set(ByVal Value As ConversionInfo)
            myGraphicInfoWPF = Value
        End Set
    End Property
...
#Region "Scrollbar"
    ...
    DefaultValue(False)>
    Public Property ShowScrollbarsWPF() As Boolean
        Get
            Return AutoScroll
        End Get
        Set(ByVal Value As Boolean)
            ...
            myIsChangingAutoScrollWPF = True
            AutoScroll = Value
            myIsChangingAutoScrollWPF = False
        End Set
    End Property
...
#Region "Events"
    Public Shadows Event MouseClick(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.MouseEventArgs, ByVal LogicalCoord As Point, ByVal CurrentClickAction As ToolType)
    Public Shadows Event MouseMove(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.MouseEventArgs, ByVal LogicalCoord As Point, ByVal CurrentClickAction As ToolType)
    Public Shadows Event MouseDown(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.MouseEventArgs, ByVal LogicalCoord As Point, ByVal CurrentClickAction As ToolType)
    Public Shadows Event MouseUp(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.MouseEventArgs, ByVal LogicalCoord As Point, ByVal CurrentClickAction As ToolType)
    Public Shadows Event MouseEnter(ByVal sender As CanvasControlWPF, ByVal e As System.EventArgs)
    Public Shadows Event MouseLeave(ByVal sender As CanvasControlWPF, ByVal e As System.EventArgs)
    Public Shadows Event Paint(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.PaintEventArgs)

    Public Event OnMeasureCompleted(ByVal sender As CanvasControlWPF, ByVal StartPoint As Point, ByVal EndPoint As Point)
    Public Event OnRedrawCompleted(ByVal sender As CanvasControlWPF, ByVal CacheRebuilded As Boolean)
    Public Event OnPictureBoxDoubleClick(ByVal sender As CanvasControlWPF, ByVal e As System.Windows.Forms.MouseEventArgs, ByVal LogicalCoord As Point)
    Public Event OnMinimumZoomLevelReached(ByVal sender As CanvasControlWPF)
    Public Event OnMaximumZoomLevelReached(ByVal sender As CanvasControlWPF)
...
#Region "Tool Implementation"
    ...
    Private Sub DrawNestedOverlays(g As Graphics, zoom As Single, pan As PointF)
        If _currentLayoutWPF Is Nothing Then Return
        If _currentLayoutWPF.Relationships Is Nothing Then Return

        For Each rel In _currentLayoutWPF.Relationships
...
#End Region
    ...
    ''' <summary>
    ''' Initializes a new instance of <see cref="CanvasControlWPF"/>.
...
#Region "Event Helper Functions"
    ...
    Protected Sub UpdateDraggingState()
        ...
        myIsDraggingWPF = (MouseButtons = MouseButtons.Left) AndAlso (Not SelectionBox.TopLeftCorner = RECT.InvalidPoint)
        If myIsDraggingWPF Then
            ...
            Dim distanceX As Integer = physicalMousePos.X - myLastMouseDownPointWPF.X
            Dim distanceY As Integer = physicalMousePos.Y - myLastMouseDownPointWPF.Y
            myIsDraggingWPF = (Math.Abs(distanceX) >= 3 OrElse Math.Abs(distanceY) >= 3)
        End If
    End Sub
    ...
    Private Function UpdateDimensions() As Boolean
        ...
        Try
            ...
            Dim bitmapCreationNeeded As Boolean = (myRedrawBackBufferWPF Is Nothing) OrElse (myRedrawBackBufferWPF.Width < newWidth) OrElse (myRedrawBackBufferWPF.Height < newHeight)
            If bitmapCreationNeeded Then
                ' Clean up any previous bitmaps
                If (myRedrawBackBufferWPF IsNot Nothing) Then myRedrawBackBufferWPF.Dispose()
                If (myRefreshBackBufferWPF IsNot Nothing) Then myRefreshBackBufferWPF.Dispose()
                ' I update the dimensions of the bitmaps I'm going to trace on
                myRedrawBackBufferWPF = New Bitmap(newWidth, newHeight)
                myRefreshBackBufferWPF = New Bitmap(newWidth, newHeight)
            End If
...
#Region "Routine for Redraw()"
    ...
    Public ReadOnly Property IsLayoutSuspended() As Boolean
        Get
            ...
            Return myIsLayoutSuspendedWPF 'OrElse Me.DesignMode
        End Get
    End Property
    ...
    Public Overridable Sub Redraw(ByVal forceGraphicCacheRebuild As Boolean)
        ...
        Try
            ...
            GR = GetScaledGraphicObject(myRedrawBackBufferWPF)
    ...
    Private Sub DrawGrid(ByVal GR As Graphics, ByVal GridInitialOffset As Integer, ByVal GridStep As Integer, ByVal GridMode As GridKind, ByVal GridColor As Color, ByVal SmartAdjust As Boolean)
        Try
            ...
            If (GridStep < myRulersWPF.GetRulerStep) AndAlso SmartAdjust Then
                GridStep = myRulersWPF.GetRulerStep
            End If
...
#Region "Input handling and rendering"
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        If _backBufferWPF Is Nothing OrElse _backBufferWPF.Size <> Me.ClientSize Then
            _backBufferWPF = New Bitmap(Me.Width, Me.Height)
            _backGraphicsWPF = Graphics.FromImage(_backBufferWPF)
        End If

        _backGraphicsWPF.SmoothingMode = SmoothingMode.AntiAlias
        _backGraphicsWPF.Clear(Me.BackColor)

        If _showGrid Then DrawGrid(_backGraphicsWPF)
        If _showRulersWPF Then DrawRulers(_backGraphicsWPF)

        For Each s In _shapes
            s.Draw(_backGraphicsWPF, _zoom, _pan)
        Next

        If _selected IsNot Nothing Then
            Using pen As New Pen(Color.DarkOrange, 2)
                pen.DashStyle = DashStyle.Dash
                _backGraphicsWPF.DrawRectangle(pen, _selected.GetBounds(_zoom, _pan))
            End Using
        End If
        ...
        If _isDrawing AndAlso _tempShape IsNot Nothing Then
            Using pen As New Pen(Color.SteelBlue, 2)
                ...
                _tempShape.Draw(_backGraphicsWPF, _zoom, _pan, pen)
            End Using
        End If
        ...
        End If
        DrawNestedOverlays(_backGraphicsWPF, _zoom, _pan)
        e.Graphics.DrawImageUnscaled(_backBufferWPF, 0, 0)
    End Sub
